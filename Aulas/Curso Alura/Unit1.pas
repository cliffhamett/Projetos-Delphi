...
begin
  if dolar.Text = '' then
    begin
      ShowMessage('Favor informar o valor em dólar!');
      dolar.SetFocus ;
    end
  else if cotacao.Text = '' then
    begin
      ShowMessage('Favor informar o valor da cotação do dia!');
      cotacao.SetFocus ;
    end
  else
    begin
...
begin
  ...
  if strtofloat(DOLAR.Text) = 0 then
    begin
      ShowMessage('O valor da compra não pode ser zero!');
      Abort ;
    end
  else if strtofloat (DOLAR.Text) < 0 then
    begin
      ShowMessage('O valor da compra não pode ser inferior a zero!');
      Abort ;
    end
  else if strtofloat (real.Text) <0 then
    begin
    ShowMessage('Clique no botão "Conversão" para recalcular o valor da compra.');
    Abort ;
  end
  else
    begin
  ...
  if VARNOME <> '' then
    begin
      Label4.Caption := 'Compra executada por ' + VARNOME + '. Você comprou ' + dolar.Text + ' dólar(es)! ' +
...
//  begin
//    showMessage('O valor informado da cotação é inválido!') ;
//    cotacao.SetFocus ;
//  end;
//end;
